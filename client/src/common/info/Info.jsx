...
class Info extends React.Component {
  ...
  componentDidUpdate() {
    if (this.showAbout) {
      ReactDOM.findDOMNode(this.about).focus()
    }
    if (this.showHelp) {
      ReactDOM.findDOMNode(this.help).focus()
    }
  }
  ...
  renderAbout() {
    return <div className={`pure-u-1 ${styles.infoText}`} ref={about => this.about = about}>
      <button onClick={this.props.toggleAbout} title='Close About' type='button'
              className={`${styles.modalButton} pure-button`}
              id='modal_close' aria-label='close'>X
      </button>
      <h1>What Is OneStop?</h1>
      The OneStop Project is designed to improve NOAA's data discovery and access framework. Focusing on all layers of
      the framework
      and not just the user interface, OneStop is addressing data format and metadata best practices, ensuring more data
      are available
      through modern web services, working to improve the relevance of dataset searches, and advancing both

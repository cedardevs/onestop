...
const UserSavedSearchList = props => {
  ...
  const propsForItem = (item, itemId) => {
    return {id: itemId, navigateToSearch: navigateToSearch, deleteSearch: deleteSearch, ...item}
  }
  ...
  const heading = props.savedSearches && props.savedSearches.length > 0 ? 'Saved Searches' : 'You have no saved searches'
  const listHeading = (

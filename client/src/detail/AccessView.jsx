...
export default class AccessView extends React.Component {
    ...
    render() {
        ...
        let information = item.links.filter(link => link.linkFunction === "information").map((link, index, arr) => {
            ...
            return (
                <div key={index}>
                    ...
                    <p style={(index === lastIndex) ? styleTableCellParagraphLast : styleTableCellParagraph}>
                        {linkDescription}
                    </p>
                </div>
            )
        ...
        let downloadData = item.links.filter(link => link.linkFunction === "download").map((link, index, arr) => {
            const lastIndex = arr.length - 1;
            const { linkUrl, linkName, linkFunction, linkDescription } = link;
            return (
                <div key={index}>
        ...
        let distributionFormatsList = <ul style={styleTableCellList}>{distributionsFormats}</ul>

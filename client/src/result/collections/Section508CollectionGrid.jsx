...
class Section508CollectionGrid extends React.Component {
  ...
  renderLinks(label, links, linkRenderer) {
    ...
    return (
      <ul title={label} className={styles.collectionList508}>
    ...
    return (
      <li key={index} className={styles.links}>
        <A
          href={linkUrl}
          target="_blank"
          title={linkProtocol || linkName || 'Link'}
        >
  ...
  renderKeyword(keyword, index) {
    return (
      <li key={index} className={styles.keywords}>
        <button
          title={keyword}
  ...
  render() {
      ...
      collections.push(
        <li {...listItemProps}>
          ...
          <div title="Related Links">
            ...
            {this.renderLinks(
              'More Info',
              this.getLinksByType('information', val.links),
              this.renderLink
            )}
            {this.renderLinks(
              'Data Access',
              this.getLinksByType('download', val.links),
              this.renderLink
            )}
          </div>
          <div title="Associated Keywords">
            ...
            {this.renderLinks(
              'Themes',
              this.getKeywordsByType(val['gcmdScience']),
              this.renderKeyword.bind(this)
            )}
            {this.renderLinks(
              'Places',
              this.getKeywordsByType(val['gcmdLocations']),
              this.renderKeyword.bind(this)
            )}
          </div>
          <div title="Associated Files">
            ...
            <button
              onClick={() => this.props.showGranules(key)}
              title="Show matching files"
    ...
    if (this.props.returnedHits < this.props.totalHits) {
      collections.push(
        <li key="showMore" className={styles.listItem}>

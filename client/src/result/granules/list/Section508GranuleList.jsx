...
class Section508GranuleList extends React.Component {
  ...
  render() {
    const granuleList = _.map(this.props.results, (value, key) => (
      <li key={key} className={styles.listItem}>
        ...
        <ul title="Access Links" className={styles.granuleList508}>
    ...
    return (
      <div
        ref={granuleFocus => (this.granuleFocus = granuleFocus)}
        tabIndex={0}
      >
        <a
          onClick={this.props.showCollections}
          title="Return To Collection Results"
          tabIndex={0}
          className={styles.links}
        >
          {' '}
          Return To Collection Results
        </a>
  ...
  renderLinks(links) {
    return _.chain(links)
      .map(link => ({
        protocol: this.identifyProtocol(link),
        url: link.linkUrl,
        name: link.linkName,
      }))
      ...
      .map(({ protocol, url, name }) => (
        <li key={url} className={styles.linkRow}>
          <A
            href={url}
            title={`${protocol.label} link`}
            className={styles.links}
          >
            {name}
          </A>

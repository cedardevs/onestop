...
describe('The geoUtils', function(){
  ...
  it('can shift an array of coordinates', function(){
    ...
    geoUtils
      .shiftCoordinates(coords, 3)
      .should.deep.equal([ [ 1070, 30 ], [ 1100, 45 ] ])
    geoUtils
      .shiftCoordinates(coords, 1)
      .should.deep.equal([ [ 350, 30 ], [ 380, 45 ] ])
    geoUtils
      .shiftCoordinates(coords, 0)
      .should.deep.equal([ [ -10, 30 ], [ 20, 45 ] ])
    geoUtils
      .shiftCoordinates(coords, -1)
      .should.deep.equal([ [ -370, 30 ], [ -340, 45 ] ])
    geoUtils
      .shiftCoordinates(coords, -3)
      .should.deep.equal([ [ -1090, 30 ], [ -1060, 45 ] ])
  })
  ...
  describe('can re-center', function(){
    ...
    it('a bbox over the date line after several rotations with decimals', function(){
      ...
      actual.coordinates[0][0][0].should.be.approximately(
        expected.coordinates[0][0][0],
        0.0000000001
      )
      actual.coordinates[0][1][0].should.be.approximately(
        expected.coordinates[0][1][0],
        0.0000000001
      )
      actual.coordinates[0][2][0].should.be.approximately(
        expected.coordinates[0][2][0],
        0.0000000001
      )
      actual.coordinates[0][3][0].should.be.approximately(
        expected.coordinates[0][3][0],
        0.0000000001
      )
      actual.coordinates[0][4][0].should.be.approximately(

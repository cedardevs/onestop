...
describe('The URL Utils', function(){
  ...
  it('identify external urls', function(){
    ...
    urls.forEach(url =>
      urlUtils.isGovExternal(url).should.equal(true, `Failed for url: ${url}`)
    )
  })
  ...
  it('identify internal urls', function(){
    ...
    urls.forEach(url =>
      urlUtils.isGovExternal(url).should.equal(false, `Failed for url: ${url}`)
    )
  })
  ...
  it('can identify the details page', function(){
    pathTests.forEach(test => {
      urlUtils
        .isDetailPage(test.path)
        .should.equal(
          test.isDetail,
          `Failed for test ${test.path} is ${test.isDetail}`
        )
    })
  })
  ...
  it('can identify the granule list page', function(){
    pathTests.forEach(test => {
      urlUtils
        .isGranuleListPage(test.path)
        .should.equal(
          test.isGranuleList,
          `Failed for test ${test.path} is ${test.isGranuleList}`
        )
    })
  })
  ...
  it('can identify the collection id on the detail page', function(){
    pathTests.forEach(test => {
      ...
      if (test.collectionIdFromDetailPath) {
        collectionId.should.equal(
          test.isDetail ? test.collectionIdFromDetailPath : null,
          `Failed for test ${test.path} expected collection id  ${test.collectionIdFromDetailPath}`
        )
      }
      else {
  ...
  it('can identify the collection id on the granule list page', function(){
    pathTests.forEach(test => {
      ...
      if (test.collectionIdFromGranuleListPath) {
        collectionId.should.equal(
          test.isGranuleList ? test.collectionIdFromGranuleListPath : null,
          `Failed for test ${test.path} expected collection id  ${test.collectionIdFromGranuleListPath}`
        )
      }
      else {
        expect(collectionId).to.be.null

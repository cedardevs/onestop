package integrationTest.org.cedar.psi.wrapper
...
class WrapperIntegrationSpec extends Specification{
    ...
    def 'retrieve raw data from output topic '() {
        def granuleText = '{"dataStream": "dscover", "trackingId": "3", "checksum": "fd297fcceb94fdbec5297938c99cc7b5", ' +
                ...
                '"fileSize": 6526, "lastUpdated":"2017124"}'
        ...
        def slurpedKey = slurper.parseText(granuleText) as Map
        String key = slurpedKey.trackingId.toString()

        // Step 1: Produce some input data to the input topic.
        IntegrationTestUtils.produceKeyValuesSynchronously(producerConfig, inputTopic , key, granuleText.toString())

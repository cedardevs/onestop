...
class ConfigUtil {
  ...
  static replaceEnv(String yamlString) {
    if (!yamlString) { return yamlString }
    return yamlString.replaceAll(/\$\{([^\s\}]+)\}/) {
  ...
  static getConfig(String name) {
    ...
    Map conf = [:]
    Yaml yaml = new Yaml()
    String configPath = System.getenv('PSI_CONFIG_LOCATION') ?
        "file://${System.getenv('PSI_CONFIG_LOCATION')}" :
        System.properties['psi.config.location'] ?: "file://" + System.getProperty("user.dir") + "/${name}.yml"
  ...
  static Map validateKafkaConfig(Map config) {
    ...
    if (!config?.application?.id) {
      throw new RuntimeException('Cannot resolve ${kafka.application.id}')
    }
    if (!config?.bootstrap?.servers) {
      throw new RuntimeException('Cannot resolve ${kafka.bootstrap.servers}')
    }
  ...
  static Map validateTopologyConfig(Map config) {
    ...
    if (!config?.input?.topic) {
      throw new RuntimeException('Cannot resolve ${stream.input.topic}')
    }
    if (!config?.split || !(config?.split instanceof List)) {
      throw new RuntimeException('Cannot resolve ${stream.split}')
    }
  ...
  static Properties streamsConfig(Map kafkaConfig) {
    log.info "Building kafka stream config: $kafkaConfig"
